...
def add_import_identities_task(trxl, backend, url, interval, args, job_id=None):
    ...
    :param interval: period of executions, in minutes. '0' to disable
    ...
    trxl.log_operation(op_type=Operation.OpType.ADD, entity_type='importIdentitiesTask',
                       timestamp=datetime_utcnow(), args=op_args,
                       target=op_args['backend'])
...
def delete_import_identities_task(trxl, task):
    ...
    trxl.log_operation(op_type=Operation.OpType.DELETE, entity_type='task',
                       timestamp=datetime_utcnow(), args=op_args,
                       target=op_args['task'])
...
def update_import_identities_task(trxl, task, **kwargs):
    ...
    (other keywords will be ignored):
       ...
       - `interval`: period of executions, in minutes. '0' to disable
    ...
    trxl.log_operation(op_type=Operation.OpType.UPDATE, entity_type='importIdentitiesTask',
                       timestamp=datetime_utcnow(), args=op_args,
                       target=op_args['task'])
