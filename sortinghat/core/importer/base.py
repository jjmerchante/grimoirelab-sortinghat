...
from .utils import find_backends
from ..db import add_import_identities_task
from ..errors import InvalidValueError
from ..jobs import import_identities
...
def create_import_task(ctx, backend_name, url, interval, params):
    ...
    klass = backends.get(backend_name)
    if not klass:
        raise InvalidValueError(msg=f"Backend '{backend_name}' does not exist.")

    if params:
        class_args = inspect_signature_parameters(klass.__init__, ('self', 'cls'))
        class_args = [arg.name for arg in class_args]
        if not all(k in class_args for k in params.keys()):
            raise InvalidValueError(msg=f"Not all arguments in 'params' are available "
                                        f"for {backend_name}")

    trxl = TransactionsLog.open('create_import_identities', ctx)

    task = add_import_identities_task(trxl=trxl, backend=backend_name, url=url,
